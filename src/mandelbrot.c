...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/07 14:34:29 by aziabkin          #+#    #+#             */
/*   Updated: 2018/07/12 21:06:41 by aziabkin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void	mandelbrot1(t_fractol *fr, t_window *wind, int x, int y)
{
	int i;

	fr->pr = 1.5 * (x - S_W / 2) / (0.5 * fr->zoom * S_W) + fr->move_x - 0.5;
	fr->pi = (y - S_H / 2) / (0.5 * fr->zoom * S_H) + fr->move_y;
	fr->new_re = 0;
	fr->new_im = 0;
	fr->old_re = 0;
	fr->old_im = 0;
	...
	while (++i < fr->max_iter)
	{
		fr->old_re = fr->new_re;
		fr->old_im = fr->new_im;
		fr->new_re = fr->old_re * fr->old_re - fr->old_im * fr->old_im + fr->pr;
